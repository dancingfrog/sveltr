...
mapHeight <- function (image_map, image_width, image_height) {
    ...
    for (y in 1:image_height) {
        for (x in 1:image_width) {
            heights[y, x] <- DISPLACE_MULTIPLY * round(image_map[y, x], 6)
            # vertices (x, y, z)
            print(paste0(round((x / image_width), 6), ", ", round(y / image_height, 6), ", ", heights[y, x])) # z is "up"
            # normals
...
mapTerrainCoords <- function (image_map, image_width, image_height) {
    ...
    for (y in 1:image_height) {
        ...
        for (x in 1:image_width) {
            ...
            # vertices (x, y, z)
            vertices[[i]] <- c(2.0 * round((x / image_width - (1.0 / image_width) - 0.5), 6), 2.0 * round((y / image_height - (1.0 / image_height) - 0.5), 6), heights[y, x])  # z is up
            # normals
