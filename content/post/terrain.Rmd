...
mapTerrainCoords <- function (image_map, image_width, image_height) {
    ...
    for (y in 1:image_height) {
        ...
        for (x in 1:image_width) {
            ...
            heights[y, x] <- round(image_map[(image_width + 1) - y, x], 6) # png y coordinate system is reverse of WebGL
            height_array_x <- c(height_array_x , heights[y, x])
            # vertices (x, y, z)
            vertices[[i]] <- c(2.0 * round((x / image_width - (1.0 / image_width) - 0.5), 6), 2.0 * round((y / image_height - (1.0 / image_height) - 0.5), 6), heights[y, x] * DISPLACE_MULTIPLY)  # z is up
            # normals
    ...
    for (y in 1:image_height) {
        for (x in 1:image_width) {
            ...
            if (x < image_width) {
                ...
                normals[[i]] <- c(round(xa, 6), normal_array[y, x, 2], normal_array[y, x, 3])
            }
            if (y < image_height) {
                za <- normal_array[y, x, 3]
                ...
                normals[[i]] <- c(normal_array[y, x, 1], round(ya, 6), normal_array[y, x, 3])
            }
        }
    }
...
Now this turns out to be a [ginormous MJS file](/post/data/terrain.mjs) (~ 20MB), so I will ultimately use the smaller heightmap as the basis of `terrain` object imported by `@sveltejs/gl`. However, it is useful to export the height data to its own json file for reference and use it to place the markers in the webapp above:
